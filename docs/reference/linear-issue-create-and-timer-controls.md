...
One shared **New issue** `NSWindow` (not inline on the island or stuffed into the popover). Identifier must not be treated as a Settings placeholder (`LaunchWindowPolicy` / Today desk pattern: dedicated `NSWindow` identifier, e.g. `PokeTaskBar.NewLinearIssue`).
