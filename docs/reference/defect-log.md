...
- **표시 카피와 스케일된 임계는 같은 숫자를 써야 한다.** `EconomyScale.tokens(5_000_000)` 이 부화
  임계를 50K 로 줄였는데 첫 실행 안내가 하드코딩 `~5M XP` 를 말하면, 남은 양(50K)과 안내가 어긋나
  "토큰이 안 쌓인다"로 오인된다. 토큰 적립 경로(`update` → `creditEarnedXP(fromTokens: true)` →
  `eggUsage`)는 Linear 시드/완료와 독립이다. 가드는 토큰만으로 알이 자라고 부화하는지
  (`testTokenUsageWithoutLinearCompletionsGrowsAndHatchesEgg`)와 안내가 스케일된 compact 값을
  쓰는지(`eggFirstRunHint(_:)`)를 함께 본다. Linear XP 는 그대로 둔다.
...
- **크기 상한이 없는 사용자 파일을 통째로 읽지 마라.** `String(contentsOf:)` 가 파일 크기만큼, 뒤따르는
  ...
  **opt-in(`POKETASKBAR_RUN_LARGE_PERF=1` / `scripts/perf-codex-large-rollout.sh`)이라 CI 는 돌리지 않는다.**
...
- **번역 가드는 "이미 표에 있는 문구"만 본다 — 표에 *못 들어간* 문구는 구조상 안 보인다.**
  ...
  `Sources/PokeTaskBar/UI/**` 의 문자열 리터럴에 한글이 있으면 실패. 한국어 *주석*은 하우스 스타일이라
