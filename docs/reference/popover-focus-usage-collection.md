...
Default size **360×640**. System light/dark via `MenuBarPanelMetrics` dynamic fills (Linear `#F3F4F6` / white in light, matching split in dark). Cards: ~12pt continuous corners, `cardFill` and a `TahoeHairline` (0.5pt from `MenuBarPanelMetrics.hairline`). Buttons, idle tabs, accessory icons, chips, and cards all keep that hairline — not only the selected state. Lists stay **opaque and unboxed** — rows use hairline dividers and a hover fill, not a card per item. Root tabs sit on the shell toolbar. Sub-tabs live **inside** the inset panel (`TahoeTabBar` / `linearSegmentChrome`: filled control surface, brighter text, soft hairline). Dropdowns are quiet bordered chips (`TahoePopupMenu` / `linearChipChrome`), tinted to match status/priority. Issue IDs are muted text, not pills. Pause / Mark done / primary actions use the same filled Linear chip (`tahoeButtonStyle(.prominent)`). SF Pro. Caption2 tertiary section labels. Clock: large rounded `monospacedDigit`.
