...
- **스크린샷(`assets/`)**: UI(`Sources/PokeTaskBar/UI/`) 변경 시 재생성. 기존 방식 = **HTML 렌더**
...
- GitHub Release zip only — PokeTaskBar v1 has no Homebrew cask.
...
GitHub Release → zip (cask 없음) → Pages를 순서대로 수행한다. 빌드 단계에서 기존 로컬 앱 종료·교체도 수행한다.
노트나 기여자가 누락되면 앱 교체·push 전에 중단하며, 게시에는 검증한 노트 파일을 그대로 사용한다.

## 3. 검증

완료 후 GitHub Release zip을 받아 `/Applications/PokeTaskBar v1.app` 버전이 새 버전인지 확인한다.
PokeTokenBar v3 설치는 그대로 둔다.
