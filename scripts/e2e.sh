...
APP="/Applications/PokeTaskBar.app"
LOG="$HOME/Library/Logs/PokeTaskBar.log"
SNAP="$HOME/Library/Application Support/PokeTaskBar/last-snapshot.json"
...
sleep 2
INSTANCES=$(pgrep -x PokeTaskBar | wc -l | tr -d ' ')
case "$INSTANCES" in
  0) bad "프로세스 없음"; exit 1 ;;
  1) ok "프로세스 생존, 단일 인스턴스 (pid $(pgrep -x PokeTaskBar))" ;;
  ...
  *) bad "인스턴스 $INSTANCES 개 — 중복 실행 (pid $(pgrep -x PokeTaskBar | tr '\n' ' '))" ;;
esac
...
var status = 0, tall = 0
for w in list where (w[kCGWindowOwnerName as String] as? String) == "PokeTaskBar" {
...
AXCNT=$(osascript -e 'tell application "System Events" to tell process "PokeTaskBar" to get count of menu bar items of menu bar 2' 2>/dev/null)
if [[ "${AXCNT:-}" =~ ^[0-9]+$ ]]; then
...
press() { osascript -e 'tell application "System Events" to tell process "PokeTaskBar" to perform action "AXPress" of menu bar item 1 of menu bar 2' >/dev/null 2>&1; }
