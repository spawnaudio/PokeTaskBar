...
**Install:** Download `PokeTaskBar.zip` below and open `PokeTaskBar v1.app`.

**Upgrade:** Download the latest release zip. PokeTaskBar v1 has no Homebrew cask.
"""
...
class ReleaseMetadataTests(unittest.TestCase):
    ...
    def test_all_sections_and_install_instructions_are_required(self):
        for value in ("## New", "## Fixed", "## Other", "## Contributors", "PokeTaskBar v1 has no Homebrew cask"):
    ...
    def test_release_stops_before_side_effects_when_notes_are_missing(self):
        ...
        commands.mkdir()
        git = commands / "git"
        git.write_text('#!/bin/sh\nif [ "$1" = rev-parse ]; then echo Master; else exit 99; fi\n')
        git.chmod(0o755)
