...
버전 배포 시 **코드뿐 아니라 문서(README·웹페이지)까지 일관되게** 갱신하기 위한 런북.
...
4. **빌드 + zip** (`build/PokeTaskBar.zip`) + 빌드 버전 일치 확인 — **push 전 검증**(실패해도 범프 미커밋이라 origin/Master 무손상).
5. **커밋 + push** (`git push origin Master`, 빌드 성공 후).
6. **GitHub Release** 생성 (`PTB_NOTES_FILE` 원문 사용, 최소 노트 자동 대체 없음).
7. **Homebrew cask skipped** — PokeTaskBar v1 has no tap.
8. **GitHub Pages 재빌드** 요청 (랜딩 동적 배지 갱신 유도).

> `Master` 브랜치에서만 실행(스크립트가 가드). 비-Master 에서 실행 시 즉시 중단.
...
- [v2.5.3](https://github.com/chattymin/PokeTaskBar/releases/tag/v2.5.3)을 기준으로 영어
...
- [ ] **GitHub Release zip** — `PokeTaskBar.zip` / `PokeTaskBar v1.app`. Homebrew cask 없음.
...
GitHub Release에서 `PokeTaskBar.zip`을 받아 `PokeTaskBar v1.app`이 새 버전인지 확인한다.
`/Applications/PokeTaskBar v1.app` 과 PokeTokenBar v3 설치는 서로 덮어쓰지 않아야 한다.

## 서명 (2026-07-08 부터)

릴리스 빌드는 이 머신의 `PokeTaskBar Local` 자체서명 인증서로 서명된다
