# Contributing to PokeTaskBar

**English** · [한국어](CONTRIBUTING.ko.md) · [日本語](CONTRIBUTING.ja.md)

Thanks for your interest in contributing! PokeTaskBar is a small, non-commercial
...
1. Create a feature branch off `Master` (fork the repo if you don't have write
   access).
...
`Master`, so English PRs keep the public history consistent.
...
- **UI changes** (anything under `Sources/PokeTaskBar/UI/`) should describe the
...
- **Adding a usage source** (a new AI CLI) = implement the `UsageProvider`
  protocol (`Sources/PokeTaskBar/Core/UsageProvider.swift`) in one new type and
  register it in the default `providers:` array of `UsageStore.init`
  (`Sources/PokeTaskBar/Core/UsageStore.swift`). Those are the only two places
...
PokeTaskBar is an **unofficial, non-commercial fan project** and is not
