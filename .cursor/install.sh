#!/usr/bin/env bash
#
# Cloud Agent install step for PokeTaskBar.
#
# PokeTaskBar is a macOS-only menu bar app: its sources import Apple-only
...
ERR
  ...
  # PokeTaskBar builds natively on macOS. A debug build both verifies the
...
Note: PokeTaskBar is a macOS-only application. `swift build`, `swift test`,
...
case "$os" in
  ...
  Linux)  install_linux ;;
  *)
    echo "✗ Unsupported OS for PokeTaskBar development: $os" >&2
    exit 1
    ;;
esac
