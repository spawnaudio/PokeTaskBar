...
final class PokemonBalanceTests: XCTestCase {
    ...
    func testEconomyScaleAppliesToOfficialTokenCosts() {
        ...
        XCTAssertEqual(RareCandy.price, EconomyScale.coins(500_000_000))
...
final class CompanionStoreTests: XCTestCase {
    ...
    /// Buying a shop egg does not release the partner, so a representative pick stays.
    func testFreshEggKeepsRepresentativeSelectionOfReleasedSpecies() throws {
        ...
        XCTAssertEqual(s.representativeSpeciesID, 1, "알 구매가 파트너를 놓아주지 않으므로 선택이 유지된다")
    ...
    // MARK: Shop eggs do not release the partner

    /// Buying an egg mid-evolution banks the egg; reached forms stay on the raising partner.
    func testReleasingMidChainCreditsOnlyReachedForms() throws {
        ...
        XCTAssertEqual(s.state.active?.pathIDs, [1, 2])
        XCTAssertEqual(s.dexSpecies.map(\.id), [1, 2], "파트너가 그대로라 같은 두 종")
        XCTAssertTrue(s.state.dex.isEmpty, "놓아줌 기록이 생기지 않는다")
        XCTAssertFalse(s.dexSpecies.contains { $0.id == 3 }, "미도달 진화형은 보유가 아니다")
        XCTAssertEqual(s.storedCompanions.count, 1)
    }

    /// Buying an egg does not dump a disguised shiny into the dex.
    func testReleasingDisguisedDittoKeepsShinyHidden() throws {
        ...
        XCTAssertFalse(s.currentIsShiny, "위장 중이면 리빌 전까지 숨김")
        XCTAssertTrue(s.state.dex.isEmpty)
    }
