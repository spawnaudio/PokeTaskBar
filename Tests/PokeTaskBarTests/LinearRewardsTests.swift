...
final class LinearRewardsTests: XCTestCase {
    ...
    private func project(_ id: String, at offset: TimeInterval = 0) -> LinearCompletedProject {
        LinearCompletedProject(
            id: id,
            name: "Ship \(id)",
            completedAt: Date(timeIntervalSince1970: 1_700_000_000 + offset))
    }

    func testFirstProjectPollSeedsWithoutXP() {
        ...
        XCTAssertEqual(outcome.xp, 0)
        XCTAssertTrue(outcome.seeded)
        XCTAssertEqual(Set(outcome.creditedIDs), ["p-a", "p-b"])
        XCTAssertTrue(outcome.newlyCredited.isEmpty)
    }

    func testSubsequentProjectPollAwardsLargePacket() {
        let seeded = LinearRewards.evaluateProjects(
            projects: [project("p-a")], alreadyCredited: [], seeded: false)
        let next = LinearRewards.evaluateProjects(
            projects: [project("p-a"), project("p-b")],
            alreadyCredited: seeded.creditedIDs,
            seeded: true)
        XCTAssertEqual(LinearRewards.xpPerProject, 10 * LinearRewards.xpPerIssue)
        XCTAssertEqual(next.xp, LinearRewards.xpPerProject)
        XCTAssertEqual(next.newlyCredited.map(\.id), ["p-b"])
    }

    func testParseCompletedProjectsFromGraphQLFixture() throws {
        let json = """
        {"data":{"projects":{"nodes":[
          {"id":"proj-1","name":"Ship it","completedAt":"2026-09-06T12:00:00.000Z"}
        ]}}}
        """.data(using: .utf8)!
        let projects = try LinearClient.parseCompletedProjects(json)
        XCTAssertEqual(projects.count, 1)
        XCTAssertEqual(projects[0].id, "proj-1")
        XCTAssertEqual(projects[0].name, "Ship it")
    }
    ...
    private func liveContainersFixture() -> Data {
        ...
        {"data":{
          "projects":{"nodes":[
            ...
            {"id":"p-prod","name":"PokeTaskBar - Adjustments",
             "status":{"type":"started","name":"Production"},
             "issues":{"nodes":[]}},
