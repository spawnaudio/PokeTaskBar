...
final class LanguageSurfaceRegressionTests: XCTestCase {
    ...
    func testVisibleCopyAndSettingsControlsAreRoutedToLocalization() throws {
        ...
        let settings = try String(contentsOf: root.appendingPathComponent("Sources/PokeTaskBar/UI/SettingsView.swift"), encoding: .utf8)
        ...
        let usage = try String(contentsOf: root.appendingPathComponent("Sources/PokeTaskBar/UI/UsageTabView.swift"), encoding: .utf8)
        for label in ["in", "out", "cache w", "cache r"] {
            XCTAssertFalse(usage.contains("tokenTypeLabel(\"\(label)\""))
        }
        let companion = try String(contentsOf: root.appendingPathComponent("Sources/PokeTaskBar/UI/CompanionView.swift"), encoding: .utf8)
