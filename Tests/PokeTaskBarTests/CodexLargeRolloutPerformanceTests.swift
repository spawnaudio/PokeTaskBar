...
final class CodexLargeRolloutPerformanceTests: XCTestCase {
    ...
    func testCodexLargeRolloutStaysWithinMemoryBudget() throws {
        guard ProcessInfo.processInfo.environment["POKETASKBAR_RUN_LARGE_PERF"] == "1" else {
            throw XCTSkip("Set POKETASKBAR_RUN_LARGE_PERF=1 or run scripts/perf-codex-large-rollout.sh")
        }
