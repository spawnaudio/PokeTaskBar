...
final class DifficultyTests: XCTestCase {
    ...
    func testShopGateAndChargedAmountFollowDifficulty() async {
        ...
        let normal = await hatched(shop: 1.0, used: EconomyScale.xpForCoins(halfPrice))
        XCTAssertFalse(normal.canBuy(.rareCandy), "1.0x: 절반 잔액으론 못 산다")

        let cheap = await hatched(shop: 0.5, used: EconomyScale.xpForCoins(halfPrice))
    ...
    func testEggPriceScalesAndIsActuallyCharged() async {
        let s = await hatched(shop: 0.5, used: EconomyScale.xpForCoins(FreshEgg.price))
