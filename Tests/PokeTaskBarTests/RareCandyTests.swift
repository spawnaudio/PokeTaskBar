...
final class RareCandyStoreTests: XCTestCase {
    ...
    func testBatchCarriesXPThroughMultipleEvolutions() async throws {
        ...
        XCTAssertEqual(preview.carryoverXP, 1_750_000)
        ...
        XCTAssertEqual(s.candyFeedbackAmount, 3 * RareCandy.xp)
        XCTAssertEqual(s.state.usedSinceInstall, before)
    }

    func testBatchCapsAtGraduationAndPreservesUnusedCandies() async throws {
        for remaining in [3_750_000, 4_500_000] {
            let s = store(rcNoEvo)
            await s.hatch(baseID: 20)
            s.applyUsage(PokemonBalance.graduationTotal(.common) - remaining)
            giveCandies(s, 5)
            ...
            XCTAssertEqual(preview.discardedXP, 3 * RareCandy.xp - remaining)
    ...
    func testBatchCapsAtInventoryAndRejectsNonpositiveAmounts() async throws {
        ...
        XCTAssertTrue(preview.evolves)
        XCTAssertEqual(s.useRareCandy(count: Int.max), .evolved)
        XCTAssertEqual(s.state.active?.usedAtStage, preview.carryoverXP)
        XCTAssertEqual(s.rareCandyCount, 0)
    }
    ...
    func testBatchPreviewMatchesGrowthWithDifficultyAndRepeatBonus() async throws {
        for difficulty in [0.1, 0.5, 1.0, 2.0] {
            for boosted in [false, true] {
                ...
                var s = CompanionStore(provider: StubProvider(value: rcLinear3), clock: { rcNow },
                                       fileURL: url, rng: SeededRNG(seed: 7), defaults: defaults)
                await s.hatch(baseID: 1)
                if boosted {
                    // Existing saves can contain boosted partners; new duplicate hatches are blocked.
                    var saved = s.state
                    saved.active?.hasGrowthBoost = true
                    try JSONEncoder().encode(saved).write(to: url)
                    s = CompanionStore(provider: StubProvider(value: rcLinear3), clock: { rcNow },
                                       fileURL: url, rng: SeededRNG(seed: 7), defaults: defaults)
                    s.update(todayTokensByProvider: [:], todayDate: "2026-09-19", monthTotal: 0,
                             burnTier: .idle, limitWarning: false, hasUsageData: false)
                    for _ in 0..<100 where s.currentLine == nil { await Task.yield() }
