...
final class PremiumEggTests: XCTestCase {
    ...
    private func eggStore(tier: Rarity?, seed: UInt64, provider: any PokeProviding,
                          used: Int = 10_000_000) -> CompanionStore {
        let f = url()
    ...
    func testPricesFollowGraduationTotalRatio() {
        XCTAssertEqual(FreshEgg.price(guaranteeing: nil), EconomyScale.coins(1_000_000_000))
        XCTAssertEqual(FreshEgg.price(guaranteeing: .uncommon), EconomyScale.coins(2_500_000_000))
        XCTAssertEqual(FreshEgg.price(guaranteeing: .rare), EconomyScale.coins(4_000_000_000))
    ...
    func testBuyPremiumEggBanksGuaranteeWithoutReleasing() {
        let s = activeStore()
        ...
        if case .egg(_, let tier, let usage) = s.storedCompanions[0] {
            XCTAssertEqual(tier, .rare)
            XCTAssertEqual(usage, 0)
        } else {
            XCTFail("rare egg should be in storage")
        }
        XCTAssertTrue(s.state.dex.isEmpty)
        XCTAssertTrue(s.state.collectedFinals.isEmpty)
    }

    /// 알을 품고 있어도 보관함 알은 살 수 있다.
    func testCanBuyEggWhileIncubating() {
        let s = eggStore(tier: nil, seed: 1, provider: TieredProvider(), used: 5_000_000_000)
        XCTAssertFalse(s.hasActive)
        XCTAssertTrue(s.isEgg)
        for tier in FreshEgg.shopTiers {
            XCTAssertTrue(s.canBuyEgg(tier), "\(String(describing: tier))")
        }
    ...
    func testFundsAreCheckedAgainstTierPrice() {
        let s = activeStore(used: EconomyScale.xpForCoins(EconomyScale.coins(3_000_000_000)))
    ...
    func testPurchaseStartsFromCleanRollState() {
        ...
        XCTAssertTrue(s.buyEgg(.rare))
        XCTAssertNil(s.state.eggTier, "육성 슬롯의 보증은 바꾸지 않는다")
        XCTAssertNil(s.state.pendingHatchID)
        if case .egg(_, let tier, _) = s.storedCompanions[0] {
            XCTAssertEqual(tier, .rare)
        } else {
            XCTFail("stored egg missing")
        }
    }
    ...
    func testGuaranteeSurvivesRestart() {
        ...
        XCTAssertEqual(s2.storedCompanions.count, 1, "재시작 후에도 보관함 알 유지")
        if case .egg(_, let tier, _) = s2.storedCompanions.first {
            XCTAssertEqual(tier, .uncommon)
        } else {
            XCTFail("stored egg missing after restart")
        }
    ...
    /// 보증도 소비되지 않는다. 보관함 알 구매는 빠져나갈 수단이 되지만, 품은 알이 벽돌이면
    /// 육성 슬롯을 비울 수 없다. **디코드되는지가 아니라 실제로 부화하는지**를 검증한다.
