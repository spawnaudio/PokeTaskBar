...
final class AppLogTests: XCTestCase {
    ...
    func testFlushDrainsAQueuedWriteBeforeReturning() {
        let queue = DispatchQueue(label: "poketaskbar.log.test")
        let landed = LineSink()
    ...
    func testFlushDrainsEveryWriteQueuedBeforeIt() {
        let queue = DispatchQueue(label: "poketaskbar.log.test.multi")
        let landed = LineSink()
    ...
    func testWriteAndFlushLandsTheLineBeforeReturning() {
        let queue = DispatchQueue(label: "poketaskbar.log.test.pair")
        let landed = LineSink()
        let log = AsyncLineLog(queue: queue) { line in
            Thread.sleep(forTimeInterval: 0.03)
            landed.append(line)
    ...
    func testAppLogWriteAndFlushRoutesThroughBackend() throws {
        let source = try String(contentsOf: repositoryFile("Sources/PokeTaskBar/Core/AppLog.swift"))
    ...
    func testMarkCleanUsesWriteAndFlushNotWriteAlone() throws {
        let source = try String(contentsOf: repositoryFile("Sources/PokeTaskBar/Core/CrashReporter.swift"))
        let body = try XCTUnwrap(
    ...
    func testDuplicateYieldUsesWriteAndFlushBeforeTerminate() throws {
        let source = try String(contentsOf: repositoryFile("Sources/PokeTaskBar/PokeTaskBarApp.swift"))
        let body = try XCTUnwrap(
    ...
    private func repositoryFile(_ relative: String) -> URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // PokeTaskBarTests
