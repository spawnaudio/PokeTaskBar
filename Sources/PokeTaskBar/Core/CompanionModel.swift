...
enum RareCandy {
    ...
    /// Shop price in Coins (1% fork scale, then ÷ 1000).
    static let price = EconomyScale.coins(500_000_000)
}

/// 민트 밸런스 상수 — timed XP multiplier, not a nature reroll.
enum Mint {
    /// Shop price in Coins (1% fork scale, then ÷ 1000).
    static let price = EconomyScale.coins(100_000_000)
    static let xpMultiplier = 2
...
enum ShinyCharm {
    ...
    static let price = EconomyScale.coins(3_000_000_000)
...
/// Shop eggs — bag items that bank in Pokémon Storage. Buying one does not release the partner.
enum FreshEgg {
    /// Shop price in Coins (1% fork scale, then ÷ 1000).
    static let price = EconomyScale.coins(1_000_000_000)
...
/// Banked eggs and mid-evolution partners. Graduated Pokémon stay on the Dex and cannot train.
enum PokemonStorageItem: Codable, Equatable, Identifiable, Sendable {
    case egg(id: String, tier: Rarity?, usage: Int)
    case partner(id: String, mon: MonState)

    var id: String {
        switch self {
        case .egg(let id, _, _): return id
        case .partner(let id, _): return id
        }
    }

    var isEgg: Bool {
        if case .egg = self { return true }
        return false
    }
}
...
/// 현재 키우는 포켓몬.
struct MonState: Codable, Equatable, Sendable {
    var baseID: Int
...
struct CompanionState: Codable, Sendable {
    // 토큰: 설치 이후만 측정. Token usage converts 1:1 into XP.
    var installBaselineSet = false
    var usedSinceInstall = 0
    /// Extra XP (Linear work, projects, time-open, mint surplus). Not token stats.
    var bonusXP = 0
    // Shop spend ledger in Coins. Wallet = floor((usedSinceInstall + bonusXP) / 1000) − spentTokens.
    var spentTokens = 0
    /// True when the training slot is empty (after graduation, before adopting from storage).
    var trainingEmpty = false
    /// Banked eggs and mid-evolution partners.
    var pokemonStorage: [PokemonStorageItem] = []
    /// Mint 2× XP window. Nil / past = inactive. Does not change identity.
    var mintExpiresAt: Date? = nil
    ...
    init(from decoder: Decoder) throws {
        ...
        linearCreditedProjectIDs = c.lenient([String].self, forKey: .linearCreditedProjectIDs, default: [])
        linearProjectSeeded = c.lenient(Bool.self, forKey: .linearProjectSeeded, default: false)
