...
///
/// crash.log 의 기록은 **다음 실행 때 메인 로그로 합쳐 비운다**(사용자는 PokeTaskBar.log 한 곳만 봐도 됨).
enum CrashReporter {
    ...
    static func install(version: String) {
        ...
        if FileManager.default.fileExists(atPath: markerURL.path) {
            AppLog.write("⚠️ 직전 세션이 정상 종료되지 않았습니다(크래시·OOM·강제종료 추정). "
                + "원인 스택은 ~/Library/Logs/DiagnosticReports/PokeTaskBar-*.ips 참조.")
        }
        try? Data().write(to: markerURL, options: .atomic)   // 이번 세션 running 마커
        AppLog.write("launch: PokeTaskBar \(version) 시작")
