...
final class CompanionStore {
    ...
    /// Mint activation flash — identity (nature / shiny / stage) does not change.
    private(set) var mintFeedbackSeq = 0
    func consumeMintFeedback() {}
    ...
    static func defaultURL() -> URL {
        // 상태 파일 위치. 기본은 Application Support/PokeTaskBar. `PTB_STATE_DIR` 환경변수가 있으면
    ...
    // 알 인큐베이션 (active 없을 때)
    var isEgg: Bool { state.active == nil && !state.trainingEmpty }
    ...
    var displayName: String {
        guard let a = state.active, let line = currentLine else { return l.trainingEgg }
        ...
        if !state.installBaselineSet {
            ...
            if hasCurrentProviderData {
                ...
                if state.claimedTodayTokensByProvider == nil {
                    ...
                    if delta > 0 {
                        creditEarnedXP(delta, fromTokens: true)
                    }
                } else {
    ...
    // MARK: 민트 (timed XP multiplier)

    /// Usable whenever stock remains. Does not require an active partner and does not
    /// change nature, shiny, species, or evolution stage.
    var canUseMint: Bool { itemCount(.mint) > 0 }

    var mintIsActive: Bool {
        guard let exp = state.mintExpiresAt else { return false }
        return clock() < exp
    }

    var mintMultiplier: Int { mintIsActive ? Mint.xpMultiplier : 1 }

    var mintRemaining: TimeInterval {
        guard let exp = state.mintExpiresAt else { return 0 }
        return max(0, exp.timeIntervalSince(clock()))
    }

    /// Activate 2× XP for 30 minutes. Identity is untouched.
    @discardableResult
    func useMint() -> Bool {
        guard canUseMint else { return false }
        state.inventory[ItemKind.mint.rawValue] = itemCount(.mint) - 1
        state.mintExpiresAt = clock().addingTimeInterval(Mint.duration)
        mintFeedbackSeq += 1
        save()
        return true
    }

    private func mintScaled(_ delta: Int) -> Int {
        guard delta > 0 else { return 0 }
        return delta * mintMultiplier
    }
    ...
    func creditEarnedXP(_ delta: Int, fromTokens: Bool, source: XPReward.Source? = nil) {
        ...
        if fromTokens {
            ...
            if xp > delta { state.bonusXP += xp - delta }
        } else {
            state.bonusXP += xp
        }
        applyGrowth(xp)
    ...
    var availableCoins: Int {
        max(0, EconomyScale.coinsFromXP(lifetimeXP) - state.spentTokens)
    }
    /// Shop wallet in Coins. Kept as `availableTokens` for existing call sites.
    ...
    /// 알은 보관함 아이템이라 활성 포켓몬이 없어도 살 수 있다.
    ...
    // MARK: Eggs → Pokémon Storage (does not release the current partner)

    /// Guarantee badge for the egg currently in the training slot.
    var eggGuarantee: Rarity? { isEgg ? state.eggTier : nil }
    ...
    /// Buy an egg into Pokémon Storage. The active partner stays in training.
    @discardableResult
    func buyEgg(_ tier: Rarity?) -> Bool {
        guard canBuyEgg(tier) else { return false }
        state.spentTokens += price(of: .egg(tier))
        state.pokemonStorage.append(.egg(id: UUID().uuidString, tier: tier, usage: 0))
        save()
        return true
    }
    ...
    /// Move the selected storage item into training. The current trainee (egg or partner) banks.
    @discardableResult
    func swapFromStorage(_ id: String) -> Bool {
        guard let idx = state.pokemonStorage.firstIndex(where: { $0.id == id }) else { return false }
        let incoming = state.pokemonStorage.remove(at: idx)
        packTrainingIntoStorage()
        unpackIntoTraining(incoming)
        save()
        return true
    }
    ...
    private func packTrainingIntoStorage() {
        if let a = state.active {
            ...
            currentLine = nil
            activeGeneration += 1
        } else if !state.trainingEmpty {
            state.pokemonStorage.append(
                .egg(id: UUID().uuidString, tier: state.eggTier, usage: state.eggUsage))
            state.eggUsage = 0
            state.eggTier = nil
            state.pendingHatchID = nil
            prefetchedLineID = nil
    ...
    private func unpackIntoTraining(_ item: PokemonStorageItem) {
        switch item {
        case .egg(_, let tier, let usage):
            state.active = nil
            state.eggTier = tier
    ...
    /// Completing a project is a large XP packet, not another issue-sized tick.
    @discardableResult
    func creditLinearProjects(_ projects: [LinearCompletedProject]) -> LinearRewards.ProjectOutcome {
        let outcome = LinearRewards.evaluateProjects(
            projects: projects,
            alreadyCredited: state.linearCreditedProjectIDs,
            seeded: state.linearProjectSeeded)
    ...
    func hatchIfNeeded() async {
        guard !state.trainingEmpty, state.active == nil, !isHatching, state.eggUsage >= eggHatchThreshold else { return }
    ...
    /// 아무도 재시도하지 않으면 다음 update 틱(기본 120초)까지 이름이 알로 남는다.
    ...
    private func ensureEggPrefetch() async {
        guard !state.trainingEmpty, state.active == nil, !isHatching, !prefetchInFlight else { return }
        let generation = activeGeneration
    ...
    private func hatchCore(baseID: Int) async {
        ...
        if state.ownsSpecies(line.baseID) && !isShiny {
    ...
    func applySave(_ envelope: SaveEnvelope, todayTokensByProvider: [String: Int], todayDate: String,
        ...
        displayState = state.active != nil ? .idle : (state.trainingEmpty ? .idle : .egg)
        migratePokemonProfilesIfNeeded()
        save()
