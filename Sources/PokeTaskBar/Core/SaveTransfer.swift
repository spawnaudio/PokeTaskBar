...
struct SaveEnvelope: Codable, Sendable {
    static let formatID = "poketaskbar.save"
...
enum SaveTransfer {
    ...
    static func suggestedFileName(date: Date) -> String {
        "PokeTaskBar-Save-\(dayStamp(date)).json"
    }
    ...
    static func sanitized(_ state: CompanionState) -> CompanionState {
        ...
        // 그 보증으로 미리 뽑아둔 종(pendingHatchID)도 함께 버린다 — 보증만 지우면 다음 육성 알이
        // 그 pre-roll 로 부화해, 아무도 사지 않은 프리미엄 결과가 나온다.
        ...
        // 않는다. 상점 알은 보관함으로 가서 육성 슬롯을 바꾸지 않으므로, 깨진 보증을 여기서 떨어
        // 육성 슬롯이 막히지 않게 한다. 디코드는 *성공*하므로 load() 의 .corrupt 복구도 안 걸려
        // 파일을 손으로 지우기 전엔 앱을 못 쓴다.
    ...
    static func rebasedForThisDevice(_ imported: CompanionState,
        ...
        state.linearCreditedProjectIDs = LinearRewards.mergedCreditedIDs(
            imported.linearCreditedProjectIDs, current.linearCreditedProjectIDs)
        state.linearProjectSeeded = imported.linearProjectSeeded || current.linearProjectSeeded
